  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm mt-1">Start a session from your plan or create a custom one.</p>
      </div>

      {/* Active session recovery */}
      {activeSession && (
        <div className="mb-6 bg-amber-50 border border-amber-200 rounded-xl p-4">
          <p className="text-amber-700 font-semibold text-sm">Session in progress: {activeSession.name}</p>
          <p className="text-gray-500 text-xs mt-1">You have an unfinished session. Resume or discard it.</p>
      ...
      <Card className="mb-4">
        ...
        {templates.length === 0 ? (
          <p className="text-gray-500 text-sm text-center py-4">No templates yet. Create one in the Planner.</p>
        ) : (
          <div className="space-y-2">
            {templates.map(t => (
              <div key={t.id} className="flex items-center justify-between bg-gray-50 rounded-lg p-3">
                <div>
                  <p className="text-sm font-medium text-gray-800">{t.name}</p>
                  <p className="text-xs text-gray-500">{t.estimatedDurationMinutes}min · {t.category}</p>
                </div>
      ...
      <button
        ...
        className="w-full border-2 border-dashed border-gray-200 hover:border-vitality-400 rounded-xl p-6 flex flex-col items-center gap-2 text-gray-400 hover:text-vitality-600 transition-colors group"
      >
        <PlusCircle className="w-8 h-8 group-hover:text-vitality-600" />
        <span className="text-sm font-medium">Start Empty Session</span>
        <span className="text-xs text-gray-400">Add exercises as you go</span>
      </button>
    </div>
  );
}
